itPlayApp.controller('OutputExercisesTabController', function($scope, LessonsFactory, OutputExercisesFactory){
	$scope.outputs = [{outputLine: ''}];
	$scope.$on('lesson_set', function(event) {
		$scope.exercises = OutputExercisesFactory.exercises($scope.lesson.$id);
		$scope.exercises.$loaded(function(data){
			$scope.exercise = $scope.exercises[0];
			if ($scope.exercise) {
				$scope.updateExercisesSelect();
			}
		});
	});
